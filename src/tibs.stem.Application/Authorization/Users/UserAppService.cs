    ...
    public class UserAppService : stemAppServiceBase, IUserAppService
    {
        ...
        public async Task<GetUserForEditOutput> GetUserForEdit(NullableIdDto<long> input)
        {
            ...
            else
            {
                ...
                output.Root = new UserRootDto
                {
                    Id = output.User.RootId,
                    Name = Sales(output.User.RootId),
                };
        ...
        public ListResultDto<TargetListDto> GetTarget(GetTargetInput input)
        {

            var Target = _TargetRepository.GetAll().Where(p => p.UserId == input.UserId);
            //var querys = Target
            // .WhereIf(
            // !input.Filter.IsNullOrEmpty(),
            // p => p.TargetType.Name.Contains(input.Filter) ||
            //   p.Users.UserName.Contains(input.Filter) ||
            //     p.TargetAmount.ToString().Contains(input.Filter)
            // );
            var query = (from s in Target
                         select new TargetListDto
                         {
                             UserId = s.UserId,
                             UserName = s.Users.UserName,
                             TargetAmount = s.TargetAmount,
                             TargetTypeId = s.TargetTypeId,
                             TargetTypeName = s.TargetType.Name,
                ...
                return new ListResultDto<TargetListDto>(list.MapTo<List<TargetListDto>>());

        }
        public async Task<PagedResultDto<TargetListDto>> GetTarget1(GetTargetInput1 input)
        {
            var Target = _TargetRepository.GetAll().Where(p => p.UserId == input.UserId);
            var querys = Target
             .WhereIf(
            ...
            var  query = (from s in querys
                ...
                var Targetlistoutput = Targetlist.MapTo<List<TargetListDto>>();

                return new PagedResultDto<TargetListDto>(TargetCount, Targetlistoutput);
        }
        ...
        public virtual async Task CreateTarget(CreateTargetInput input)
        {
            ...
            var targetTotal = _TargetRepository.GetAll().Where(a => a.TenantId == input.TenantId && a.ValidityDate.Month == input.ValidityDate.Month && a.ValidityDate.Year == input.ValidityDate.Year).Select(p => p.TargetAmount).Sum();
            ...
            else
            {
                ...
                var query = _TargetRepository.GetAll().Where(p => p.UserId == input.UserId && p.TenantId == input.TenantId && Convert.ToDateTime(p.ValidityDate).Year == InputYear && Convert.ToDateTime(p.ValidityDate).Month == InputMonth).FirstOrDefault();
                if (query == null)
                {
        ...
        // [AbpAuthorize(AppPermissions.Pages_Administration_Users_TargetDelete)]
        public async Task GetDeleteTarget(EntityDto input)
        {
            await _TargetRepository.DeleteAsync(input.Id);
        }
