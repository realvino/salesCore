...
namespace tibs.stem.Web.Controllers
{
    ...
    {
        public TenantManager TenantManagers { get; set; }
        ...
        // 19.10.2017
        [HttpPost]
        public async Task<AuthenticateTenentResultModel> TenentSearching([FromBody] AuthenticateModel model)
        {
            var output = new AuthenticateTenentResultModel { };


            using (CurrentUnitOfWork.DisableFilter(AbpDataFilters.MayHaveTenant))
            {
                var user = _userManager.Users.Where(u => u.EmailAddress == model.UserNameOrEmailAddress).FirstOrDefault();
                if (user != null)
                {
                    var query = (from u in _userRoleRepository.GetAll()
                                 join t in TenantManagers.Tenants.Distinct() on u.TenantId equals t.Id
                                 where u.UserId == user.Id
                                 select t).FirstOrDefault();



                    output.UserId = (int)user.Id;
                    output.TenentId = (int)user.TenantId;
                    output.TenentName = query.TenancyName;
                }
            }
